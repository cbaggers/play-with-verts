    ...
    (labels ((edge-to-boundary-vertex (edge-index)
      ...
      (let* ((mcase
              (loop
                 ...
                 :when (> d 0)
                 :sum (expt 2 v)))
             (faces (aref *cases* mcase)))
        (loop
           :for edges :across faces
           :for verts :=
             (loop
                :for edge-index :across edges
                :do (edge-to-boundary-vertex edge-index)))
        (values)))))
...
(defun do-it ()
  (multiple-value-bind (verts norms)
      (march (lambda (x) (circle 8f0 x)))
    (assert (= (length verts) (length norms)))
    (let ((res (make-gpu-array nil
                               :dimensions (length verts)
                               :element-type 'vert)))
      (with-gpu-array-as-c-array (carr res)
        (loop
           :for v :across verts
           :for n :across norms
           :for i :from 0
           :for s := (aref-c carr i)
           :do (setf (vert-pos s) v
                     (vert-norm s) n)))
      res)))
  ...
  (let ((offset 0.01f0))
    (v3:normalize
     (v! (- (funcall func (v3:+ pos (v!     offset  0f0 0f0)))
         ...
         (- (funcall func (v3:+ pos (v! 0f0      offset 0f0)))
            (funcall func (v3:+ pos (v! 0f0  (- offset) 0f0))))

         (- (funcall func (v3:+ pos (v! 0f0  0f0    offset)))
            (funcall func (v3:+ pos (v! 0f0  0f0 (- offset)))))))))
