...
void occlusion(uint3 threadID : SV_DispatchThreadID)
{
  ...
  if (threadID.x < NoofInstances)
  {
    ...
    for (int i = 0; i < 8; i++)
    {
      ...
      float4 clipPos = mul(float4(boxCorners[i], 1),
			   ViewProjection);
    ...
    // Use the lower level if we only touch <= 2 texels in
    // both dimensions
    ...
    float4 depth =
      { inputRT.SampleLevel(samplerPoint, boxUVs.xy, mip),
    ...
    //find the max depth
    float maxDepth = max(max(max(depth.x, depth.y), depth.z),
			 depth.w);
